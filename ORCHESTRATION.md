...
**Never author a shared API surface in parallel.** This one cost a full workflow cycle. Four agents wrote four parts of one spec concurrently, and because the API they all depended on did not exist yet, each invented its own: two incompatible `DescriptorWriter` constructors, two `blitImageToSwapchain` signatures each labelled "final", two mutually exclusive upload mechanisms, and the same accessor under two names. All three verifiers returned not-ready, and the failure was structurally identical to the one the previous critique had already diagnosed.

Parallel agents can split work that touches **disjoint files**. They cannot split work that **defines a common interface**. When a phase needs both, serialize: one agent authors the API surface and nothing else, that artifact is verified, and only then do the parts fan out against it as a fixed input. The interface is the barrier — this is one of the few places a `parallel()` barrier is genuinely correct rather than lazy.

**Re-verify "unused" claims by compiling, not grepping.** An agent removed a CMake setting after grepping thoroughly and reasoning correctly about what it found, and was still wrong: the symbol was dead in the configuration it could build and load-bearing in the one it could not. Grep has a specific blind spot around conditional build paths. If a claim of deadness gates a deletion, the check is "remove it and build every configuration," and the spec should say which method was used.
